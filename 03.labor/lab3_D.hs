...
import System.Win32 (xBUTTON1, pAGE_EXECUTE)
...
myProduct5 ls = foldr (*) ls

myProduct6 ls = foldl (*) ls

myProduct7 ls = foldr' (*) 1 ls

myProduct8 ls = foldl' (*) 1 ls
...
myMaximum4 [x] = x
myMaximum4 (x : xs) = max x (myMaximum xs)
...
listaMap = map (uncurry listaN) ls3

-- listaMap2 = map (x -> listaN x 0) ls3
